...
private:
	...
	dMatrix<double> calPeelingLIK(const std::vector<unsigned int> & sampleIndex, double & LIK);
...
public:
	...
	//calculate the posterior probability before normalization and also the pedigree likelihood
	dMatrix<double> calPostProbLIK(const dMatrix<double> &lk, const std::vector<int> & sampleId, double & pedLIK);
};
